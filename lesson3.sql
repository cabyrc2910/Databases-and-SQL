...
-- SELECT firstname, age FROM staff ORDER BY firstname, age DESC ; - отсортировалось только по имени
...
-- SELECT DISTINCT id, firstname FROM staff; не правильная запись, выведет уникальность по 2 значениям полей
...
-- SELECT id FROM staff ORDER BY id LIMIT 2; выведет первые 2 записи только из колонки id
...
-- Выведите максимальную  заработные платы и минимальный возраст
SELECT MAX(salary), MIN(age) FROM staff;
