...
-- Задача 2
-- выбрать фотографии (filename) пользователя с email: arlo50@example.org . 
...
-- делаем с использованием вложеных запросов (ошибка: количество запросов больше 1 строки)
-- SELECT filename FROM media 
-- WHERE user_id = (SELECT id FROM users WHERE email = 'arlo50@example.org')
-- AND media_type_id = (SELECT id FROM media_types WHERE name_type LIKE 'photo' ); -- photo - только 1

-- Решение
-- делаем с использованием вложеных запросов
SELECT * FROM media 
WHERE user_id = (SELECT id FROM users WHERE email = 'arlo50@example.org')
AND media_type_id IN (SELECT id FROM media_types WHERE name_type LIKE 'p%' ); -- p% - все значения
...
-- INNER JOIN
-- выборка сообщений по пользователям
SELECT * FROM users u
JOIN messages m 
WHERE u.id=m.from_user_id;

-- Правильное написание для быстрой выборки
SELECT * FROM users u
JOIN messages m ON u.id=m.from_user_id;
-- WHERE firstname = 'Reuben'; -- выборка (фильтрация) по пользователю
...
FROM media m
LEFT JOIN likes l ON l.media_id = m.id -- файл сравниваем с файлом
JOIN users u ON u.id = m.user_id  -- человека сравниваем с человеком
GROUP BY m.id
ORDER BY m.id;
