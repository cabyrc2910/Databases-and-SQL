-- создаём базу данных
DROP DATABASE IF EXISTS ДЗ_4;
CREATE DATABASE ДЗ_4;
USE ДЗ_4;
...
INSERT INTO likes (user_id, media_id) VALUES 
    (1, 1), (2, 1), (3, 1), (4, 1), (5, 2), (6, 2), (7, 2), (8, 8), 
    (1, 9), (10, 15), (7, 11), (5, 12), (6, 13), 
...
SELECT count(media_id) AS like_count  from likes where media_id IN
...
SELECT CONCAT(f.firstname," ",  f.lastname) AS full_name, COUNT(m.id) AS messages_count FROM users u
...
WHERE u.id = 1
GROUP BY full_name ORDER BY messages_count DESC LIMIT 1;