...
-- Реализовать добавление нового пользователя с профилем c определение COMMIT или ROLLBACK в базу данных с помощью хранимых процедур
...
-- общий город
	SELECT p2.user_id  -- , rand() AS rand_id
	FROM profiles p1
	JOIN profiles p2 ON p1.hometown = p2.hometown 
	WHERE p1.user_id = for_user_id AND p2.user_id <> for_user_id
	AND p2.user_id NOT IN (SELECT id FROM friends)
    UNION 
-- состоят в одной группе
	SELECT DISTINCT uc2.user_id  -- , rand() AS rand_id
    ...
    AND uc2.user_id NOT IN (SELECT id FROM friends)
-- друзья друзей
    UNION
	SELECT fr.initiator_user_id   -- , rand() AS rand_id
    	FROM friends f
...
/*Задача 3. Необходимо перебрать всех пользователей и тем пользователям, у которых дата рождения меньше 
